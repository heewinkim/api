...
def P_Net(mode='train'):
    ...
    if mode == 'test':
        ...
        cls_prob = mx.symbol.SoftmaxOutput(data=conv4_1, label=label,multi_output=True, use_ignore=True,out_grad=True, name="cls_prob")
        # cls_prob = mx.symbol.SoftmaxOutput(data=conv4_1, label=label, multi_output=True, use_ignore=True,name="cls_prob")

        conv4_2_reshape = mx.symbol.Reshape(data = conv4_2, shape=(-1, 4), name="conv4_2_reshape")

        bbox_pred = mx.symbol.LinearRegressionOutput(data=conv4_2_reshape, label=bbox_target, grad_scale=1, out_grad=True, name="bbox_pred")
        # bbox_pred = mx.symbol.LinearRegressionOutput(data=conv4_2_reshape, label=bbox_target, grad_scale=1, name="bbox_pred")
