...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
            ...
            keep = self.py_nms(boxes, 0.7, 'Union')
            boxes = boxes[keep]
            ...
            keep = self.py_nms(all_boxes[:, 0:5], 0.7, 'Union')
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
        ...
        for i in range(num_boxes):
            ...
            cropped_ims[i, :, :, :] = self.transform(cv2.resize(tmp, (24, 24)))
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
        ...
        for i in range(num_boxes):
            tmp = np.zeros((tmph[i], tmpw[i], 3), dtype=np.uint8)
            tmp[dy[i]:edy[i]+1, dx[i]:edx[i]+1, :] = im[y[i]:ey[i]+1, x[i]:ex[i]+1, :]
            cropped_ims[i, :, :, :] = self.transform(cv2.resize(tmp, (48, 48)))
        ...
        keep = self.py_nms(boxes_c, 0.7, "Minimum")
